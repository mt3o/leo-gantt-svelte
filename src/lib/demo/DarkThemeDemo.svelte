<script lang="ts">
  ...
  import { GANTT_THEME_DARK } from '$lib/gantt-theme';
  ...
  const projectedTasks = $derived(mockTasks.map(task => {
    ...
    if (projection) {
      return {
        ...
        color: '#475569' // slate-600 to indicate grouped state
      };
    }
    return task;
  }));
...
<main class="dark  w-screen flex flex-col bg-slate-900 text-white overflow-hidden">
    <h1>Dark theme</h1>
  <div
    class="flex-1 p-4"
    onwheel={onWheel}
